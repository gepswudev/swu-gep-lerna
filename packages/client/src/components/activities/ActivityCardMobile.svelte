...
<button class={"btn btn-ghost w-48 h-full mx-0 "+" "+sx}>
  <img src={getImg(data.img)} alt={`${data.title} Image preview`} loading="lazy" class="w-48 h-32 object-cover rounded-xl">
...
{:else}
<button class={"btn btn-ghost w-48 h-full mx-0 "+" "+sx}>
  <img src={getImg(data.img)} alt={`${data.engTitle} Image preview`} loading="lazy" class="w-48 h-32 object-cover rounded-xl">
